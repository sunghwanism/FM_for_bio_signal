...
class DeepSense(nn.Module):
    ...
    def init_encoder(self):
        ...
        self.class_layer = nn.Sequential(nn.Linear(self.config["embedding_dim"]*2, self.config["fc_dim"]),
                                        nn.GELU(),
        ...
        for mod in self.modalities:
            if mod == "ecg":
                ...
                nn.Linear(self.config["recurrent_dim"] * 2, self.args.trainer_config["embedding_dim"]),
                nn.ReLU(),
                nn.Linear(self.args.trainer_config["embedding_dim"], self.args.trainer_config["embedding_dim"]),
            )
            else:
                self.mod_projectors[mod] = nn.Sequential(
