...
class MesaActigraphyService(object):
    ...
    def load_raw(file_id):
        ...
        with open(project_root + 'mesa/overlap/mesa-actigraphy-psg-overlap.csv') as csv_file:
        ...
        with open(project_root + 'mesa/actigraphy/mesa-sleep-' + file_id + '.csv') as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=',')
            next(csv_file)
            for row in csv_reader:
